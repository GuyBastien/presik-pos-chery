...
`presik-pos_6.0.75+1_amd64.deb` prêt à être installé.
...
```bash
echo "6.0.75+1" > VERSION
cat VERSION
```

Tu dois voir `6.0.75+1`.
...
`6.0.75+1`, et tes mises à jour partent à l'envers. Tu peux vérifier :

```bash
dpkg --compare-versions "6.0.75+2" gt "6.0.75+1" && echo "ordre correct"
```
...
```
dpkg-deb: construction du paquet « presik-pos » dans « dist/presik-pos_6.0.75+1_amd64.deb ».
```
...
```bash
dpkg-deb -f dist/presik-pos_6.0.75+1_amd64.deb Package Version Maintainer
```
...
```bash
dpkg-deb --fsys-tarfile dist/presik-pos_6.0.75+1_amd64.deb > /dev/null && echo "archive OK"
dpkg-deb -c dist/presik-pos_6.0.75+1_amd64.deb | wc -l
```

Tu dois voir `archive OK`, puis un nombre autour de 4565. Aucun message d'erreur.
...
```bash
dpkg-deb --fsys-tarfile dist/presik-pos_6.0.75+1_amd64.deb \
...
```bash
sudo apt install ./dist/presik-pos_6.0.75+1_amd64.deb
dpkg -l presik-pos
md5sum dist/presik-pos_6.0.75+1_amd64.deb
```
...
```
Dépaquetage de presik-pos (6.0.75+1) ...
Paramétrage de presik-pos (6.0.75+1) ...
...
```
ii  presik-pos  6.0.75+1  amd64  Client point de vente Presik POS (build local)
```
...
Tu dois voir une fenêtre qui demande la langue, Français ou Español. Ton choix
est gardé dans `~/.config/presik_pos/language.conf`. Pour en changer plus tard,
passe par le menu **Opciones**, entrée **Idioma / Langue**.

## Étape 2. Renseigner le serveur

À l'écran de connexion, ouvre le menu **Opciones**, puis **Servidor / Serveur**.
...
**Le choix de la langue au démarrage**, gardé dans
`~/.config/presik_pos/language.conf`.
...
162 Mo, 4565 fichiers, architecture amd64.
